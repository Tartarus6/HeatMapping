...
const MAX_WALK_TRANSFER_DISTANCE: f32 = 1000.0;
...
fn main() {
    let initial_depart_instant: DepartInstant = DepartInstant {
        position: Position {
            // Amsterdam
            lat: 0.913998595445,
            lon: 0.085599725524,
    ...
    let gtfs_data = get_gtfs_data(&initial_depart_instant);
    println!("Initializing: {}ms\n", now.elapsed().as_millis());

    // Shader
    shader::run(gtfs_data, initial_depart_instant).block_on();
}
